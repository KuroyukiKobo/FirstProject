...
from fastapi import Depends, status # status を使うとHTTPステータスコードが分かりやすい
...
async def get_weather(city_name: str):
    ...
    if not api_key:
        ...
        raise HTTPException(status_code=500, detail="APIキーが設定されておらぬ！バックエンドの.envファイルを確認するのじゃ。")
    ...
    try:
        ...
        if data.get("cod") == "404":
            raise HTTPException(status_code=404, detail=f"指定された都市 '{city_name}' の天候は見つからぬ！")

        # 必要な情報を抽出して、定義したPydanticモデルの形式で返すのじゃ
        temperature = data["main"]["temp"]
        description = data["weather"][0]["description"]
        icon_code = data["weather"][0]["icon"]
        icon_url = f"http://openweathermap.org/img/wn/{icon_code}@2x.png" # 天気アイコンのURL
        ...
        raise HTTPException(status_code=500, detail=f"外部APIへのリクエストに失敗したのじゃ: {e}")
    except Exception as e:
        # その他の予期せぬエラーを捕捉するのじゃ
        raise HTTPException(status_code=500, detail=f"天候情報の取得中に予期せぬエラーが発生したのじゃ: {e}")
...
def create_new_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    if db_user:
        raise HTTPException(status_code=400, detail="そのユーザー名は既に登録されておる！")
